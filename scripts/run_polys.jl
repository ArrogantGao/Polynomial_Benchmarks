...
function main()
	...
	for T in [Float32, Float64]
      ...
      x0 = T(0.9)
			estrin_runtime = @belapsed $poly_estrin($x0)
			horner_runtime = @belapsed horner_rule($x0, $poly_estrin)
			polynomial_runtime = @belapsed $poly($x0)
      @show T, n, estrin_runtime, horner_runtime, polynomial_runtime
			df = DataFrame(n = n, T = T, et = estrin_runtime, ht = horner_runtime, pt = polynomial_runtime)
